...
class ManagementofFrames(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (Home_Page, QuizTime): # This is essnentially saying, for every item in the group, frame is equal to it, and its container.
...
class QuizTime(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.QuizHeader =tk.Label(self,font=15,text="Welcome to the Quiz! Please press Yes or No on each question")
	...
	self.ResultsNotification = tk.Label(self, text="The Quiz will automatically load to the results once finished")
	#self.GotoResults = Button(self, text="Results are ready",command=,height = 5, width = 20)
	...
	if x < 4:
		...
		self.ResultsNotification.grid_forget()
	...
	if x < 4:
		...
		# Grid_Forget hides them from being visible to the user, we then display their results.
		self.YesButton.grid_forget()
		self.NoButton.grid_forget()
		self.ResultsNotification.grid_forget()
		self.QuizHeader.grid_forget()
